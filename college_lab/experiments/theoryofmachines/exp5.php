...
<body>
  <div class="container">
    <!-- Main Form -->
      <form id="exp5-form" method="post" onsubmit="event.preventDefault(); submitExperiment();" class="form-section">
  ...
  <script>
    ...
    function previewExp() {
      ...
      const previewHtml = `
        ...
        <h3>Nomenclature of Gears:</h3>
        ${Array.from(form.querySelectorAll("textarea")).slice(4).map(el => `<p><b>${el.previousElementSibling.textContent}:</b> ${formatTextWithBreaks(el.value)}</p>`).join("")}
      `;
    ...
    function submitExperiment() {
      ...
      const employee_id = '123'; // Example validation
      ...
      if (!form.gear.value.trim() || !form.function.value.trim() || !form.classification.value.trim() || !form.spur.value.trim()) {
