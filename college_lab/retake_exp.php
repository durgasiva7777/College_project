...
$sql = "SELECT submission_id, experiment_id, verification_date, submission_data 
        FROM submissions 
        WHERE student_id = ? AND verification_status = 'Retake'
        ORDER BY verification_date DESC";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $student_id);
$stmt->execute();
$retake_experiments = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt->close();
?>
...
<link rel="stylesheet" href="css/retake_exp.css">
</head>
<body>
...
<div class="container">
    ...
    <div class="content">
        <div class="profile-container">
            ...
            <?php if (count($retake_experiments) > 0): ?>
                <?php foreach ($retake_experiments as $exp): ?>
                <div class="experiment-item">
                    <h3>Experiment ID: <?php echo htmlspecialchars($exp['experiment_id']); ?></h3>
                    <div class="date">Marked for retake on: <?php echo htmlspecialchars($exp['verification_date']); ?></div>
                    <div><?php echo $exp['submission_data']; ?></div>
                </div>
                <?php endforeach; ?>
            <?php else: ?>
                <p class="no-experiments">No retake experiments found.</p>
            <?php endif; ?>
        </div>
    </div>
</div>
